...
@Component({
  selector: 'app-bao-cao-qt13b-du-lieu-kh-cr-theo-dai-ly',
  templateUrl: './bao-cao-qt13b-du-lieu-kh-cr-theo-dai-ly.component.html',
  styleUrls: ['./bao-cao-qt13b-du-lieu-kh-cr-theo-dai-ly.component.scss']
})
export class BaoCaoQt13bDuLieuKhCrTheoDaiLyComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.reportService.GhiNhanLogSuDungMenu(
      "CP_LogSuDungMenu", this.userGetData, "QT13b - Báo cáo dữ liệu khách hàng Cr theo đại lý"
    ).subscribe(t => {});
    ...
    this.comboxService.GetComboxFunction('CP_Viewdmtp').subscribe(
      t => {
    ...
    for(let i=1;i<5;i++)
    {
      this.bangDRA.push({ "ma_dra": i.toString() });
    }

    this.arrTrangThaiLoad[4] = true;
    this.draVal = "";
    this.isQuanHuyen = false;
    this.fillData();
  ...
  fillData() {
    ...
    this.reportData1 = this.reportService.CP_WKPI_CR_02B(
      this.khoiVal, this.capVal,this.draVal, sTuNgay,sDenNgay,quanhuyenval,this.thanhPhoVal, "","", this.donviVal, this.userGetData
    );
